...
class UserViewsTestCase(TestCase):
    ...
    def test_login_failure(self):
        ...
        resp = self.client.post(
            '/login', data=wrong_username_data, follow_redirects=True)
        html = resp.get_data(as_text=True)

        self.assertEqual(resp.status_code, 200)
        self.assertIn('id="user_form"', html)
        self.assertIn('Bad login.', html)
        ...
        resp = self.client.post(
            '/login', data=wrong_password_data, follow_redirects=True)
        html = resp.get_data(as_text=True)
        ...
        bad_data = {
            'username': 'dne13r113r',
            'password': 'nopeioaefaef'
        }

        resp = self.client.post(
            '/login', data=bad_data, follow_redirects=True)
        html = resp.get_data(as_text=True)
    ...
    def test_list_users(self):
        ...
        self.assertEqual(resp.status_code, 200)
        # Try to test using id's
        self.assertIn('@testuser2', html)

    def test_show_user_following_failure(self):
        """ Test that if not logged in, redirects to home page with error
        message.
        """
    ...
    def test_add_follow_failure(self):
        """Test to try to follow without logging in redirects to home page with
        error message.
        """
    ...
    def test_stop_follow_failure(self):
        ...
        resp = self.client.post(
            f'/users/stop-following/{self.u1_id}', follow_redirects=True)
        html = resp.get_data(as_text=True)
    ...
    def test_edit_profile_success(self):
        ...
        resp = self.client.post(
            f'/users/profile', data=edit_data, follow_redirects=True)
        html = resp.get_data(as_text=True)
